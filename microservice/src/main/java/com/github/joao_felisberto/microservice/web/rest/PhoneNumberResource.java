...
/**
 * REST controller for managing {@link com.github.joao_felisberto.microservice.domain.PhoneNumber}.
 */
@RestController
@RequestMapping("/api/DEPRECATED/phone-numbers")
@Transactional
public class PhoneNumberResource {

    private static final Logger LOG = LoggerFactory.getLogger(PhoneNumberResource.class);

    private static final String ENTITY_NAME = "microservicePhoneNumber";
    ...
    public ResponseEntity<PhoneNumber> createPhoneNumber(@Valid @RequestBody PhoneNumber phoneNumber) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/phone-numbers/" + phoneNumber.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, phoneNumber.getId().toString()))
            .body(phoneNumber);
    }
    ...
    public ResponseEntity<PhoneNumber> updatePhoneNumber(
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, phoneNumber.getId().toString()))
            .body(phoneNumber);
    }
    ...
    public ResponseEntity<PhoneNumber> partialUpdatePhoneNumber(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, phoneNumber.getId().toString())
        );
    }
     ...
     */
    @GetMapping("")
    public List<PhoneNumber> getAllPhoneNumbers(@RequestParam(name = "filter", required = false) String filter) {
        if ("client-is-null".equals(filter)) {
            LOG.debug("REST request to get all PhoneNumbers where client is null");
            return StreamSupport.stream(phoneNumberRepository.findAll().spliterator(), false)
                .filter(phoneNumber -> phoneNumber.getClient() == null)
                .toList();
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<PhoneNumber> getPhoneNumber(@PathVariable("id") Long id) {
